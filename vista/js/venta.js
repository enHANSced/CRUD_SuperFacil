...
function guardarFactura() {
 ...
 var selectedOption = $("#tipo_pago").val();

    if (selectedOption === "Efectivo") {
        var montoEfectivo = parseFloat(prompt("Ingrese el monto en efectivo:"));

        if (!isNaN(montoEfectivo)) {
            var totalAPagar = parseFloat($("#total").text());

            if (montoEfectivo >= totalAPagar) {
                var cambio = montoEfectivo - totalAPagar;
                alert("Venta realizada exitosamente.\nEl cambio es: " + cambio.toFixed(2));
            } else {
                alert("Monto insuficiente. Ingrese un monto igual o mayor al total a pagar.");
                // Si el monto es insuficiente, puedes tomar medidas adicionales, como cancelar la venta o volver a solicitar el monto.
                return; // Salir de la función si el monto es insuficiente
            }
        } else {
            alert("Ingrese un monto en efectivo válido.");
            // Si el monto ingresado no es válido, puedes tomar medidas adicionales, como cancelar la venta o volver a solicitar el monto.
...
function restarProductos() {
    ...
    $("#tblventa tbody tr").each(function () {
        ...
        var cantidadVendida = $(this).find("td:eq(3)").text();
